...
void Chare::check() {
  if (list)
  {
    ...
    //list->recurse(this, &Member::setContainer);
...
void UChare::check() {
	if (list) 
	{
    list->recurse(this, &Member::setUChare);
	}
}

void
UChare::print(XStr& str)
{
  if(external)
    str << "extern ";
  //if(templat)
...
UChare::genDecls(XStr& str)
{
  ...
  //Forward declaration of the user-defined implementation class*/
  str << tspec(false)<<" class "<<type<<";\n";
  str << tspec(false)<<" class "<<Prefix::Base<<type<<";\n";
  str << tspec(false)<<" class "<<Prefix::Proxy<<type<<";\n";
  str << "/* --------------- uchare base class ------------------ */\n";
  str << tspec()<< "class "<<Prefix::Base<<type;
  if(external || type->isTemplated())
  { //Just a template instantiation/forward declaration
    str << ";";
  }
  ...
  { //Actual implementation
    str << CIClassStart;
    if(list)
      list->recurse<XStr&>(str, &Member::genBaseDecls);
    str << CIClassEnd;
  }

  str << "/* --------------- uchare proxy class ------------------ */\n";
  str << tspec()<< "class "<<Prefix::Proxy<<type;
	str << ": public " << Prefix::Base << type << ", ";
	str << "public uChare_Proxy<" << type << "," << Prefix::Proxy << type << "," << Prefix::Base << type << ">\n";
  if(external || type->isTemplated())
  { //Just a template instantiation/forward declaration
    str << ";";
  }
  ...
  { //Actual implementation
    str << CIClassStart;
		str << "   " << Prefix::Proxy << type << "() {}\n";
		str << "   " << Prefix::Proxy << type;
		str << "(uChareSet<" << type << "," << Prefix::Proxy << type << "," << Prefix::Base << type << "> *uchareset)\n";
		str << "      : uChare_Proxy<" << type << "," << Prefix::Proxy << type << "," << Prefix::Base << type << "> (uchareset) ";
		str << "{\n";
		str << "";
    if(list)
      list->recurse<XStr&>(str, &Member::genRegUnpack);
		str << "   }\n";
		str << "   " << "void initEntryFunctions() {\n"; 
		str << "      " << "clearEntryFunctions();\n"; 
    if(list)
      list->recurse<XStr&>(str, &Member::genRegUnpack);
		str << "   " << "}\n";
		// pup method
		str << "   " << "void pup(PUP::er &p) {\n";
	  str << "      "	<< "initEntryFunctions();\n";
		str << "      " << "uChare_Proxy<" << type << "," << Prefix::Proxy << type << "," << Prefix::Base << type << ">::pup(p);\n";
		str << "   " << "}\n";
    if(list)
      list->recurse<XStr&>(str, &Member::genUChareProxyTriples);
    str << CIClassEnd;
  }

  str << "/* --------------- uchare base class ------------------ */\n";
	str << "typedef uChareAttr<" << type << "," << Prefix::Proxy << type << "," << Prefix::Base << type << "> " << 
		"uChareAttr_" << type << ";\n";
  str << tspec() << "class " << Prefix::Base << "uChare_" << type;
	str << ": public " << Prefix::Base << type << ", public uChare <" << type << "," << Prefix::Proxy << type << "," << Prefix::Base << type << ">\n";
  if(external || type->isTemplated())
  { //Just a template instantiation/forward declaration
    str << ";";
  }
  ...
  { //Actual implementation
    str << CIClassStart;
		...
		// Construstors
  	str << "   " << Prefix::Base << "uChare_" << type;
		str << "(" << "const " << "uChareAttr_" << type << " &attr) :\n";
		str << "      " << "uChare<" << type << "," << Prefix::Proxy << type << "," << Prefix::Base << type << ">(attr) " << "{}\n";

    str << CIClassEnd;
  }
  str << "/* --------------- uchare array proxy class ------------------ */\n";
  str << tspec() << "class " << Prefix::Proxy << "uChare_" << type;
	str << ": public uChareArray<" << type << "," << Prefix::Proxy << type << "," << Prefix::Base << type << ">\n";
  if(external || type->isTemplated())
  { //Just a template instantiation/forward declaration
    str << ";";
  }
  ...
  { //Actual implementation
    str << CIClassStart;
		...
		// Construstors
  	str << "   " << Prefix::Proxy << "uChare_" << type << "() {}\n";
  	str << "   " << Prefix::Proxy << "uChare_" << type;
		str << "(" << "const " << "uChareArray <" << type << "," << Prefix::Proxy << type << "," << Prefix::Base << type << "> &uchareArray) :\n";
  	...
  	str << "   " << "static " << Prefix::Proxy << "uChare_" << type << " ckNew (int nElements, int nChareSets) {\n";
		str << "      " <<  "return " << "uChareArray <" << type << "," << Prefix::Proxy << type << "," << Prefix::Base << type << ">" 
			<< "::ckNew (\n"; 
		str << "         " << "uChareArrayOpts<" << type << "," << Prefix::Proxy << type << "," << Prefix::Base << type << ">" 
			<< "(nElements, nChareSets)" << "); " << "}\n";

		// operator []
		str << "   " << Prefix::Proxy << type << " *operator [] (std::size_t i) { user_proxy.set_dst(i); return &user_proxy;} \n";

		// flush
		str << "   " << "void flush() { user_proxy.flush(); } \n";

		// print collective wrappers
    if(list)
      list->recurse<XStr&>(str, &Member::genUChareArrayWrappers);
