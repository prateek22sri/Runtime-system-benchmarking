...
static __inline__ int a2a_sched_linear(int rank, int p, MPI_Aint sndext,
                                       MPI_Aint rcvext, NBC_Schedule *schedule,
                                       void *sendbuf, int sendcount,
                                       MPI_Datatype sendtype, void *recvbuf,
                                       int recvcount, MPI_Datatype recvtype,
                                       NBC_Comminfo *comm);
static __inline__ int
a2a_sched_pairwise(int rank, int p, MPI_Aint sndext, MPI_Aint rcvext,
                   NBC_Schedule *schedule, void *sendbuf, int sendcount,
                   MPI_Datatype sendtype, void *recvbuf, int recvcount,
                   MPI_Datatype recvtype, NBC_Comminfo *comm);
static __inline__ int a2a_sched_diss(int rank, int p, MPI_Aint sndext,
                                     MPI_Aint rcvext, NBC_Schedule *schedule,
                                     void *sendbuf, int sendcount,
                                     MPI_Datatype sendtype, void *recvbuf,
                                     int recvcount, MPI_Datatype recvtype,
...
int NBC_Alltoall_args_compare(NBC_Alltoall_args *a, NBC_Alltoall_args *b,
                              void *param) {

  if ((a->sendbuf == b->sendbuf) && (a->sendcount == b->sendcount) &&
      (a->sendtype == b->sendtype) && (a->recvbuf == b->recvbuf) &&
      (a->recvcount == b->recvcount) && (a->recvtype == b->recvtype)) {
    return 0;
  }
...
int NBC_Ialltoall(void *sendbuf, int sendcount, MPI_Datatype sendtype,
  ...
  if (alg == NBC_A2A_DISS) {
    ...
    if (NBC_Type_intrinsic(sendtype)) {
      ...
      memcpy(handle->tmpbuf, (char *)sendbuf + datasize * rank,
             datasize * (p - rank));
      if (rank != 0)
        memcpy((char *)handle->tmpbuf + datasize * (p - rank), sendbuf,
               datasize * (rank));
    } else {
      int pos = 0;

      /* non-contiguous - pack */
      res = MPI_Pack((char *)sendbuf + rank * sendcount * sndext,
                     (p - rank) * sendcount, sendtype, handle->tmpbuf,
                     (p - rank) * datasize, &pos, MPI_COMM_WORLD);
      if (MPI_SUCCESS != res) {
      ...
      if (rank != 0) {
        pos = 0;
        MPI_Pack(sendbuf, rank * sendcount, sendtype,
                 (char *)handle->tmpbuf + datasize * (p - rank),
                 rank * datasize, &pos, MPI_COMM_WORLD);
        if (MPI_SUCCESS != res) {
          printf("MPI Error in MPI_Pack() (%i)\n", res);
          return res;
        }
  ...
  found = (NBC_Alltoall_args *)hb_tree_search(
      (hb_tree *)handle->comminfo->NBC_Dict[NBC_ALLTOALL], &search);
  if (found == NULL) {
#endif
    ...
    switch (alg) {
      case NBC_A2A_LINEAR:
        res = a2a_sched_linear(rank, p, sndext, rcvext, schedule, sendbuf,
                               sendcount, sendtype, recvbuf, recvcount,
                               recvtype, comm);
        break;
      case NBC_A2A_DISS:
        res = a2a_sched_diss(rank, p, sndext, rcvext, schedule, sendbuf,
                             sendcount, sendtype, recvbuf, recvcount, recvtype,
                             handle, comm);
        break;
      case NBC_A2A_PAIRWISE:
        res = a2a_sched_pairwise(rank, p, sndext, rcvext, schedule, sendbuf,
                                 sendcount, sendtype, recvbuf, recvcount,
                                 recvtype, comm);
        break;
    }
    ...
    res = hb_tree_insert((hb_tree *)handle->comminfo->NBC_Dict[NBC_ALLTOALL],
                         args, args, 0);
    if (res != 0)
      printf("error in dict_insert() (%i)\n", res);
    /* increase number of elements for A2A */
    if (++handle->comminfo->NBC_Dict_size[NBC_ALLTOALL] >
        NBC_SCHED_DICT_UPPER) {
      NBC_SchedCache_dictwipe(
          (hb_tree *)handle->comminfo->NBC_Dict[NBC_ALLTOALL],
          &handle->comminfo->NBC_Dict_size[NBC_ALLTOALL]);
      /*if(!rank) printf("[%i] removing %i elements - new size: %i \n", rank,
       * SCHED_DICT_UPPER-SCHED_DICT_LOWER,
       * handle->comminfo->NBC_Alltoall_size);*/
    }
...
static __inline__ int a2a_sched_diss(int rank, int p, MPI_Aint sndext,
   ...
   * (unsigned long)handle->tmpbuf, (int)datasize*p*(1<<maxround), maxround);*/
  for (r = 1; r < p; r <<= 1) {
    offset = 0;
    for (i = 1; i < p; i++) {
      ...
      if ((i & r) == r) {
         ...
         * i, (unsigned long)(stmpbuf+offset));*/
        NBC_Sched_copy((void *)(long)(i * datasize), true, datasize, MPI_BYTE,
                       stmpbuf + offset - (unsigned long)handle->tmpbuf, true,
                       datasize, MPI_BYTE, schedule);
        offset += datasize;
      }
    }
     ...
     * offset, rpeer, (unsigned long)rtmpbuf);*/
    res = NBC_Sched_recv(rtmpbuf - (unsigned long)handle->tmpbuf, true, offset,
                         MPI_BYTE, rpeer, schedule, comm);
    if (NBC_OK != res) {
      printf("Error in NBC_Sched_recv() (%i)\n", res);
      return res;
    }
     ...
     * speer, (unsigned long)stmpbuf);*/
    res = NBC_Sched_send(stmpbuf - (unsigned long)handle->tmpbuf, true, offset,
                         MPI_BYTE, speer, schedule, comm);
    if (NBC_OK != res) {
      printf("Error in NBC_Sched_send() (%i)\n", res);
      return res;
    }
    ...
    for (i = 1; i < p; i++) {
      ...
      if ((i & r) == r) {
        ...
        NBC_Sched_copy(rtmpbuf + offset - (unsigned long)handle->tmpbuf, true,
                       datasize, MPI_BYTE, (void *)(long)(i * datasize), true,
                       datasize, MPI_BYTE, schedule);
        offset += datasize;
      }
    }
  ...
  for (i = 0; i < p; i++) {
    ...
    res = NBC_Sched_unpack((void *)(long)(i * datasize), true, recvcount,
                           recvtype, rbuf, false, schedule);
    if (NBC_OK != res) {
