...
void compute_moments(node_t *node, particle_t *P, int n_parts) {
...
double node_compute_approx(node_t *node, double pos) {
...
double node_compute_direct(particle_t *P, int count, double pos) {
  double retval{0.0};
...
int save_and_continue_moments_handler(node_t *node, hpx_addr_t moments) {
  hpx_lco_get(moments, sizeof(moment_t), &node->moments);
...
int partition_node_handler(node_t *node, hpx_addr_t parts, int n_parts,
                           int n_partition);
HPX_ACTION(HPX_DEFAULT, HPX_PINNED,
...
int partition_node_handler(node_t *node, hpx_addr_t parts, int n_parts,
  ...
  if (n_parts <= n_partition) {
    ...
    particle_t *P_end = &P_begin[n_parts];

    double x_split = 0.5 * (node->low + node->high);
    auto x_comp = [x_split](particle_t &a) {
      return a.pos < x_split;
    };
    particle_t *splitter = std::partition(P_begin, P_end, x_comp);
    ...
    hpx_addr_t reduce = hpx_lco_reduce_new(2, sizeof(moment_t),
    ...
    if (n_parts_left) {
      ...
      hpx_addr_t parts_left = hpx_gas_alloc_local_at_sync(1,
                    sizeof(particle_t) * n_parts_left, 0, node->left);
      hpx_addr_t cpy_done = hpx_lco_future_new(0);
      assert(cpy_done != HPX_NULL);
      hpx_gas_memcpy(parts_left, parts, sizeof(particle_t) * n_parts_left,
                     cpy_done);
      ...
      moment_t empty{0.0, 0.0, 0.0};
      hpx_lco_set_rsync(reduce, sizeof(moment_t), &empty);
    }

    if (n_parts_right) {
      node->right = create_node(x_split, node->high);
      hpx_addr_t parts_right = hpx_gas_alloc_local_at_sync(1,
                    sizeof(particle_t) * n_parts_right, 0, node->right);
      hpx_addr_t from_addx = hpx_addr_add(parts,
                                          sizeof(particle_t) * n_parts_left,
                                          sizeof(particle_t) * n_parts);
      hpx_addr_t cpy_done = hpx_lco_future_new(0);
      assert(cpy_done != HPX_NULL);
      hpx_gas_memcpy(parts_right, from_addx,
                     sizeof(particle_t) * n_parts_right, cpy_done);
      hpx_lco_wait(cpy_done);
      hpx_lco_delete_sync(cpy_done);
...
int particle_set_approx_handler(particle_t *part, hpx_addr_t phi) {
...
int node_compute_potential_handler(node_t *node, double pos, double theta);
HPX_ACTION(HPX_DEFAULT, HPX_PINNED,
...
int spawn_computation_handler(node_t *node, hpx_addr_t root, hpx_addr_t sync,
                              double theta);
HPX_ACTION(HPX_DEFAULT, HPX_PINNED,
...
int spawn_computation_handler(node_t *node, hpx_addr_t root, hpx_addr_t sync,
  ...
  if (is_leaf(node)) {
    ...
    for (int i = 0; i < node->count; ++i) {
      hpx_addr_t paddx = hpx_addr_add(node->parts, sizeof(particle_t) * i,
                                      sizeof(particle_t) * node->count);
...
hpx_addr_t create_node(double low, double high) {
  ...
  hpx_addr_t retval = hpx_gas_alloc_local_at_sync(1,
                          sizeof(node_t), 0, HPX_THERE(where));
  node_t vals{HPX_NULL, HPX_NULL, low, high,
              moment_t{0.0, 0.0, 0.0}, HPX_NULL, 0};
  hpx_gas_memput_rsync(retval, &vals, sizeof(node_t));
  return retval;
}
