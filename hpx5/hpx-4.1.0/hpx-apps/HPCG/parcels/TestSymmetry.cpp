...
/*!
  ...
  @see ComputeMG_ref
*/
int TestSymmetry(SparseMatrix & A, Vector & b, Vector & xexact, TestSymmetryData & testsymmetry_data,Domain *domain) {

 local_int_t nrow = A.localNumberOfRows;
 local_int_t ncol = A.localNumberOfColumns;

 Vector x_ncol, y_ncol, z_ncol;
 InitializeVector(x_ncol, ncol,domain->gasvectors.newlocalLength,
                                                   domain->gasvectors.reduceid,
                                                   domain->gasvectors.allreduce,
                                                   domain->params.comm_rank,
                                                   domain->params.comm_size);
 InitializeVector(y_ncol, ncol,domain->gasvectors.newlocalLength,
                                                   domain->gasvectors.reduceid,
                                                   domain->gasvectors.allreduce,
                                                   domain->params.comm_rank,
                                                   domain->params.comm_size);
 InitializeVector(z_ncol, ncol,domain->gasvectors.newlocalLength,
                                                   domain->gasvectors.reduceid,
                                                   domain->gasvectors.allreduce,
                                                   domain->params.comm_rank,
                                                   domain->params.comm_size);
 ...
 // Next, compute x'*A*y
 ComputeDotProduct(nrow, y_ncol, y_ncol, yNorm2, t4, A.isDotProductOptimized,domain);
 int ierr = ComputeSPMV(A, y_ncol, z_ncol,domain); // z_nrow = A*y_overlap

 if (ierr) HPCG_fout << "Error in call to SpMV: " << ierr << ".\n" << endl;
 double xtAy = 0.0;
 ierr = ComputeDotProduct(nrow, x_ncol, z_ncol, xtAy, t4, A.isDotProductOptimized,domain); // x'*A*y
 if (ierr) HPCG_fout << "Error in call to dot: " << ierr << ".\n" << endl;

 // Next, compute y'*A*x
 ComputeDotProduct(nrow, x_ncol, x_ncol, xNorm2, t4, A.isDotProductOptimized,domain);
 ierr = ComputeSPMV(A, x_ncol, z_ncol,domain); // b_computed = A*x_overlap
 if (ierr) HPCG_fout << "Error in call to SpMV: " << ierr << ".\n" << endl;
 double ytAx = 0.0;
 ierr = ComputeDotProduct(nrow, y_ncol, z_ncol, ytAx, t4, A.isDotProductOptimized,domain); // y'*A*x
 if (ierr) HPCG_fout << "Error in call to dot: " << ierr << ".\n" << endl;
 ...
 // Compute x'*Minv*y
 ierr = ComputeMG(A, y_ncol, z_ncol,domain); // z_ncol = Minv*y_ncol
 if (ierr) HPCG_fout << "Error in call to MG: " << ierr << ".\n" << endl;
 double xtMinvy = 0.0;
 ierr = ComputeDotProduct(nrow, x_ncol, z_ncol, xtMinvy, t4, A.isDotProductOptimized,domain); // x'*Minv*y
 if (ierr) HPCG_fout << "Error in call to dot: " << ierr << ".\n" << endl;

 // Next, compute z'*Minv*x
 ierr = ComputeMG(A, x_ncol, z_ncol,domain); // z_ncol = Minv*x_ncol
 if (ierr) HPCG_fout << "Error in call to MG: " << ierr << ".\n" << endl;
 double ytMinvx = 0.0;
 ierr = ComputeDotProduct(nrow, y_ncol, z_ncol, ytMinvx, t4, A.isDotProductOptimized,domain); // y'*Minv*x
 if (ierr) HPCG_fout << "Error in call to dot: " << ierr << ".\n" << endl;
 ...
 for (int i=0; i< numberOfCalls; ++i) {
   ierr = ComputeSPMV(A, x_ncol, z_ncol,domain); // b_computed = A*x_overlap
   if (ierr) HPCG_fout << "Error in call to SpMV: " << ierr << ".\n" << endl;
   if ((ierr = ComputeResidual(A.localNumberOfRows, b, z_ncol, residual,domain)))
     HPCG_fout << "Error in call to compute_residual: " << ierr << ".\n" << endl;
   if (A.geom->rank==0) HPCG_fout << "SpMV call [" << i << "] Residual [" << residual << "]" << endl;
 }
 DeleteVector(x_ncol,domain->params.comm_rank);
 DeleteVector(y_ncol,domain->params.comm_rank);
 DeleteVector(z_ncol,domain->params.comm_rank);
