...
double run_parallel_search(amplusplus::transport &trans, amplusplus::transport &barrier_trans, 
		...
		typename graph_traits<Graph>::vertices_size_type n, bool verify)
{
  ...
  boost::graph::distributed::parallel_search<
		Graph, ColorMap> ps(g, color, Color::black());
  ps.run(start);
	  ...
	  if (trans.rank() == 0) {
		...
		{ amplusplus::scoped_epoch epoch(trans); 
			...
			for (uint64_t i = 0; i < e_count; i++) {
				...
				//std::cout << "adding edge (" << e.first << "," << e.second << ")\n";
  ...
  DistanceMap distance = get(&VertexProperties::distance, g);
	...
	for (unsigned int source_i = 0; source_i < num_sources; ++source_i) {
		...
		//{ amplusplus::scoped_epoch epoch(trans); 
			...
			double bfs_time = 
				run_parallel_search<Graph>(trans, barrier_trans, g, current_source, color, distance, n, verify);
