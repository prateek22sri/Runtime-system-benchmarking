...
int CG(const SparseMatrix & A, CGData & data, const Vector & b, Vector & x,
    ...
    double * times, bool doPreconditioning,Domain *domain) {
...
//#endif
  ...
  CopyVector(x, p);
  TICK(); ComputeSPMV(A, p, Ap,domain); TOCK(t3); // Ap = A*p
  TICK(); ComputeWAXPBY(nrow, 1.0, b, -1.0, Ap, r, A.isWaxpbyOptimized);  TOCK(t2); // r = b - Ax (x stored in p)
  TICK(); ComputeDotProduct(nrow, r, r, normr, t4, A.isDotProductOptimized,domain); TOCK(t1);
  normr = sqrt(normr);
#ifdef HPCG_DEBUG
  ...
  for (int k=1; k<=max_iter && normr/normr0 > tolerance; k++ ) {
    ...
    if (doPreconditioning)
      ComputeMG(A, r, z,domain); // Apply preconditioner
    else
    ...
    if (k == 1) {
      ...
      TICK(); ComputeDotProduct (nrow, r, z, rtz, t4, A.isDotProductOptimized,domain); TOCK(t1); // rtz = r'*z
    } else {
      oldrtz = rtz;
      TICK(); ComputeDotProduct (nrow, r, z, rtz, t4, A.isDotProductOptimized,domain); TOCK(t1); // rtz = r'*z
      beta = rtz/oldrtz;
    ...
    TICK(); ComputeSPMV(A, p, Ap,domain); TOCK(t3); // Ap = A*p
    TICK(); ComputeDotProduct(nrow, p, Ap, pAp, t4, A.isDotProductOptimized,domain); TOCK(t1); // alpha = p'*Ap
    alpha = rtz/pAp;
