...
void GenerateProblem(SparseMatrix & A, Vector * b, Vector * x, Vector * xexact, Domain *domain)
{
  ...
  if (b!=0) InitializeVector(*b, localNumberOfRows,domain->gasvectors.newlocalLength,
                                                   domain->gasvectors.reduceid,
                                                   domain->gasvectors.allreduce,
                                                   domain->params.comm_rank,
                                                   domain->params.comm_size);
  if (x!=0) InitializeVector(*x, localNumberOfRows,domain->gasvectors.newlocalLength,
                                                   domain->gasvectors.reduceid,
                                                   domain->gasvectors.allreduce,
                                                   domain->params.comm_rank,
                                                   domain->params.comm_size);
  if (xexact!=0) InitializeVector(*xexact, localNumberOfRows,domain->gasvectors.newlocalLength,
                                                   domain->gasvectors.reduceid,
                                                   domain->gasvectors.allreduce,
                                                   domain->params.comm_rank,
                                                   domain->params.comm_size);
  ...
  generateproblem_reduction(&totalNumberOfNonzeros,localNumberOfNonzeros,
                            domain->reduce.reduction_sum,
                            domain->reduce.allreduce,
                            domain->params.comm_rank);
#if 0
#ifndef HPCG_NOMPI
