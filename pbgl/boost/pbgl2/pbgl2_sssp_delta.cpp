...
// Delta-stepping single source shortest paths algorithm
//#define PRINT_DEBUG
		...
		typename graph_traits<Graph>::vertices_size_type n, bool verify)
{
  ...
  boost::graph::distributed::delta_stepping_shortest_paths<Graph,
			...
			typename boost::property_map<Graph, weight_type WeightedEdge::*>::type>
				sssp(g, distance /*get(&VertexProperties::distance, g)*/, get(&WeightedEdge::weight, g));
		...
		if (trans.rank() == 0)
			BGL_FORALL_VERTICES_T(v, g, Graph) {
				BGL_FORALL_OUTEDGES_T(v, e, g, Graph) {
					Vertex u = target(e, g);
					weight_type we = get(get(&WeightedEdge::weight, g), e);
						std::cerr << "  Edge " << get(get(vertex_local, g), v) << "@" 
