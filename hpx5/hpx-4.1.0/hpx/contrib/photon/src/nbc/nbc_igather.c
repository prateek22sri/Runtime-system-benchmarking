...
int NBC_Igather(void *sendbuf, int sendcount, MPI_Datatype sendtype,
  ...
  search.root = root;
  found = (NBC_Gather_args *)hb_tree_search(
      (hb_tree *)handle->comminfo->NBC_Dict[NBC_GATHER], &search);
  if (found == NULL) {
#endif
    ...
    res = hb_tree_insert((hb_tree *)handle->comminfo->NBC_Dict[NBC_GATHER],
                         args, args, 0);
    if (res != 0)
      printf("error in dict_insert() (%i)\n", res);
    /* increase number of elements for A2A */
    if (++handle->comminfo->NBC_Dict_size[NBC_GATHER] > NBC_SCHED_DICT_UPPER) {
      NBC_SchedCache_dictwipe((hb_tree *)handle->comminfo->NBC_Dict[NBC_GATHER],
                              &handle->comminfo->NBC_Dict_size[NBC_GATHER]);
    }
  } else {
    /* found schedule */
