...
#include <graph500-3.0.0/generator/graph_generator.h> // Graph500 generator 
...
namespace boost {
  ...
  public:
    ...
    graph500_iterator(vertices_size_type scale, edges_size_type start_edge, 
      ...
      : scale(scale), current(start_edge), packed_edges(new packed_edge[1 << 16]), weights(new float[1 << 16])
    {
      ...
      generate_kronecker_range(seed, scale, start_edge / (1 << 16) * (1 << 16), start_edge / (1 << 16) * (1 << 16) + (1 << 16), 
					packed_edges.get(), weights.get());
      update_cache(false);
    }

    void update_cache(bool run_from_increment) {
      if (run_from_increment && current % (1 << 16) == 0) {
        generate_kronecker_range(seed, scale, current, current + (1 << 16), packed_edges.get(), weights.get());
      }
      current_edge = std::make_pair(get_v0_from_edge(&packed_edges[current % (1 << 16)]),
				    get_v1_from_edge(&packed_edges[current % (1 << 16)])); 
			current_weight = weights[current % (1 << 16)];
    }

    const std::pair<vertices_size_type, vertices_size_type>&
    dereference() const { return current_edge; }

    const std::pair<std::pair<vertices_size_type, vertices_size_type>, float>&
    dereference_weighted() const { return std::make_pair(current_edge, current_weight); }

		const float weight() const { return current_weight; }
