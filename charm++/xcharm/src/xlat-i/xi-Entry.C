...
// ucharelib
int Entry::isReentrant(void) { return (attribs & SREENTRANT); }

char *Entry::getEntryName() { return name; }
int Entry::getLine() { return line; }

void Entry::genBaseDecls(XStr& str) 
{
	if (isConstructor())
		return;

  str << "/* DECLS: "; print(str); str << " */\n";
	str << "\n    virtual " << retType << " " << name << "(";
	if (param)
		param->print(str);
	str << ") = 0;\n";
}

void Entry::genRegUnpack(XStr& str) {
	if (isConstructor())
		return;
	if (isReentrant())
		str << "      registerEntryFunction(&" << Prefix::Proxy << container->baseName() << "::"<< name << "_unpack, true);\n";
	else 
		str << "      registerEntryFunction(&" << Prefix::Proxy << container->baseName() << "::"<< name << "_unpack, false);\n";
}

void Entry::genUChareArrayWrappers(XStr& str) {
	if (isConstructor())
		return;
	str << "   " << retType << " " << name << "(";
	if (param)
	...
	//str << "      " << "for (int i = 0; i < nElements; i++) {";
	str << "      " << "for (int i = nElements-1; i >= 0; i--) {";
	str << "user_proxy.set_dst(i); ";
	str << "user_proxy." << name << "(";
	if (param)
		param->printName(str);
	str << "); }";
...
void Entry::genUChareProxyTriples(XStr& str) {
	if (isConstructor())
		return;
	str << "   " << retType << " " << name << "(";
	if (param)
	...
	str << "   " << retType << " " << name << "_pack" << "(";
	if (param)
		param->print(str);
	str << "){\n";
	str << "      " << "Message m(getFunctionId(&" <<  
		Prefix::Proxy << container->baseName() << "::"<< name << "_unpack));\n";
	if (param)
		param->addAllValuesToMessage(str);
	str << "      " << "addNewMsg(m);\n";
	
	str << "   }\n";

	str << "   " << retType << " " << name << "_unpack" << "(const Message & m){\n";
  if (param && !param->isVoid()) {
	 str << "      ";
	 param->print(str, 0, 0, 0, ';');
	 str << ";\n";
	 str << "      int parameterNumber = 0;\n";
	 param->getAllValuesFromMessage(str);	
	}

	str << "      //" << "getProxy()[m.destination]->" << name << "(";
  if (param)
	 param->printName(str);	
	str << ");\n";

	str << "      " << "getProxy().getLocalUChare(m.destination)->" << name << "(";
  if (param)
	 param->printName(str);	
	str << ");\n";
