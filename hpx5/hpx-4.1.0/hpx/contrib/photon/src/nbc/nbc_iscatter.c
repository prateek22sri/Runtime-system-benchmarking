...
int NBC_Iscatter(void *sendbuf, int sendcount, MPI_Datatype sendtype,
  ...
  search.root = root;
  found = (NBC_Scatter_args *)hb_tree_search(
      (hb_tree *)handle->comminfo->NBC_Dict[NBC_SCATTER], &search);
  if (found == NULL) {
#endif
    ...
    res = hb_tree_insert((hb_tree *)handle->comminfo->NBC_Dict[NBC_SCATTER],
                         args, args, 0);
    if (res != 0)
    ...
    if (++handle->comminfo->NBC_Dict_size[NBC_SCATTER] > NBC_SCHED_DICT_UPPER) {
      NBC_SchedCache_dictwipe(
          (hb_tree *)handle->comminfo->NBC_Dict[NBC_SCATTER],
          &handle->comminfo->NBC_Dict_size[NBC_SCATTER]);
    }
  } else {
    /* found schedule */
