...
/*!
  ...
  @see CG()
 */
int TestCG(SparseMatrix & A, CGData & data, Vector & b, Vector & x, TestCGData & testcg_data,Domain *domain) {
  ...
  InitializeVector(origDiagA, A.localNumberOfRows,domain->gasvectors.newlocalLength,
                                                   domain->gasvectors.reduceid,
                                                   domain->gasvectors.allreduce,
                                                   domain->params.comm_rank,
                                                   domain->params.comm_size);
  InitializeVector(exaggeratedDiagA, A.localNumberOfRows,domain->gasvectors.newlocalLength,
                                                   domain->gasvectors.reduceid,
                                                   domain->gasvectors.allreduce,
                                                   domain->params.comm_rank,
                                                   domain->params.comm_size);
  InitializeVector(origB, A.localNumberOfRows,domain->gasvectors.newlocalLength,
                                                   domain->gasvectors.reduceid,
                                                   domain->gasvectors.allreduce,
                                                   domain->params.comm_rank,
                                                   domain->params.comm_size);
  ...
  for (int k=0; k<2; ++k) { // This loop tests both unpreconditioned and preconditioned runs
    ...
    for (int i=0; i< numberOfCgCalls; ++i) {
      ...
      int ierr = CG(A, data, b, x, maxIters, tolerance, niters, normr, normr0, &times[0], k==1,domain);
  ...
  CopyVector(origB, b);
  // Delete vectors
  DeleteVector(origDiagA,domain->params.comm_rank); 
  DeleteVector(exaggeratedDiagA,domain->params.comm_rank);
  DeleteVector(origB,domain->params.comm_rank);
