...
int NBC_Ireduce_scatter(void *sendbuf, void *recvbuf, int *recvcounts,
    ...
    if ((rank % (1 << r)) == 0) {
      ...
      if (peer < p) {
        ...
        if (firstred) {
           ...
           */
          res = NBC_Sched_op(redbuf - (unsigned long)handle->tmpbuf, true,
                             sendbuf, false, 0, true, count, datatype, op,
                             schedule);
          firstred = 0;
        } else {
          /* perform the reduce in my local buffer */
          res = NBC_Sched_op(redbuf - (unsigned long)handle->tmpbuf, true,
                             redbuf - (unsigned long)handle->tmpbuf, true, 0,
                             true, count, datatype, op, schedule);
        }
        if (NBC_OK != res) {
          free(handle->tmpbuf);
      ...
      if (firstred) {
        ...
        res = NBC_Sched_send(redbuf - (unsigned long)handle->tmpbuf, true,
                             count, datatype, peer, schedule, comm);
      }
      if (NBC_OK != res) {
        free(handle->tmpbuf);
  ...
  if (rank != 0) {
    res = NBC_Sched_recv(recvbuf, false, recvcounts[rank], datatype, 0,
                         schedule, comm);
    if (NBC_OK != res) {
      free(handle->tmpbuf);
      printf("Error in NBC_Sched_recv() (%i)\n", res);
  ...
  if (rank == 0) {
    ...
    for (r = 1; r < p; r++) {
      ...
      res = NBC_Sched_send(sbuf - (unsigned long)handle->tmpbuf, true,
                           recvcounts[r], datatype, r, schedule, comm);
      if (NBC_OK != res) {
        free(handle->tmpbuf);
        printf("Error in NBC_Sched_send() (%i)\n", res);
        return res;
    ...
    res = NBC_Sched_copy(redbuf - (unsigned long)handle->tmpbuf, true,
                         recvcounts[0], datatype, recvbuf, false, recvcounts[0],
                         datatype, schedule);
    if (NBC_OK != res) {
      free(handle->tmpbuf);
