...
//#endif
  ...
  CopyVector(x, p);
  TICK(); ComputeSPMV_ref(A, p, Ap,domain);  TOCK(t3); // Ap = A*p
  TICK(); ComputeWAXPBY_ref(nrow, 1.0, b, -1.0, Ap, r); TOCK(t2); // r = b - Ax (x stored in p)
  TICK(); ComputeDotProduct_ref(nrow, r, r, normr, t4,domain);  TOCK(t1);
  normr = sqrt(normr);
#ifdef HPCG_DEBUG
  ...
  for (int k=1; k<=max_iter && normr/normr0 > tolerance; k++ ) {
    ...
    if (doPreconditioning)
      ComputeMG_ref(A, r, z,domain); // Apply preconditioner
    else
    ...
    if (k == 1) {
      ...
      TICK(); ComputeDotProduct_ref(nrow, r, z, rtz, t4,domain); TOCK(t1); // rtz = r'*z
    } else {
      oldrtz = rtz;
      TICK(); ComputeDotProduct_ref(nrow, r, z, rtz, t4,domain); TOCK(t1); // rtz = r'*z
      beta = rtz/oldrtz;
    ...
    TICK(); ComputeSPMV_ref(A, p, Ap,domain); TOCK(t3); // Ap = A*p
    TICK(); ComputeDotProduct_ref(nrow, p, Ap, pAp, t4,domain); TOCK(t1); // alpha = p'*Ap
    alpha = rtz/pAp;
