...
void GenerateCoarseProblem(const SparseMatrix & Af,Domain *domain)
{
  ...
  GenerateProblem(*Ac, 0, 0, 0,domain);
  SetupHalo(*Ac,domain);
  Vector *rc = new Vector;
  Vector *xc = new Vector;
  Vector * Axf = new Vector;
  InitializeVector(*rc, Ac->localNumberOfRows,domain->gasvectors.newlocalLength,
                                                   domain->gasvectors.reduceid,
                                                   domain->gasvectors.allreduce,
                                                   domain->params.comm_rank,
                                                   domain->params.comm_size);
  InitializeVector(*xc, Ac->localNumberOfColumns,domain->gasvectors.newlocalLength,
                                                   domain->gasvectors.reduceid,
                                                   domain->gasvectors.allreduce,
                                                   domain->params.comm_rank,
                                                   domain->params.comm_size);
  InitializeVector(*Axf, Af.localNumberOfColumns,domain->gasvectors.newlocalLength,
                                                   domain->gasvectors.reduceid,
                                                   domain->gasvectors.allreduce,
                                                   domain->params.comm_rank,
                                                   domain->params.comm_size);
