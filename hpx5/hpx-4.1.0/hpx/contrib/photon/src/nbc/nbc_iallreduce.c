...
static __inline__ int allred_sched_diss(int rank, int p, int count,
                                        MPI_Datatype datatype, void *sendbuf,
                                        void *recvbuf, MPI_Op op,
                                        NBC_Schedule *schedule,
                                        NBC_Handle *handle, NBC_Comminfo *comm);
static __inline__ int allred_sched_chain(int rank, int p, int count,
                                         MPI_Datatype datatype, void *sendbuf,
                                         void *recvbuf, MPI_Op op, int size,
                                         int ext, NBC_Schedule *schedule,
                                         NBC_Handle *handle, int fragsize,
                                         NBC_Comminfo *comm);
static __inline__ int allred_sched_ring(int rank, int p, int count,
                                        MPI_Datatype datatype, void *sendbuf,
                                        void *recvbuf, MPI_Op op, int size,
                                        int ext, NBC_Schedule *schedule,
                                        NBC_Handle *handle, NBC_Comminfo *comm);
...
int NBC_Allreduce_args_compare(NBC_Allreduce_args *a, NBC_Allreduce_args *b,
                               void *param) {

  if ((a->sendbuf == b->sendbuf) && (a->recvbuf == b->recvbuf) &&
...
int NBC_Iallreduce(void *sendbuf, void *recvbuf, int count,
  ...
  search.op = op;
  found = (NBC_Allreduce_args *)hb_tree_search(
      (hb_tree *)handle->comminfo->NBC_Dict[NBC_ALLREDUCE], &search);
  if (found == NULL) {
#endif
    ...
    switch (alg) {
      case NBC_ARED_BINOMIAL:
        res = allred_sched_diss(rank, p, count, datatype, sendbuf, recvbuf, op,
                                schedule, handle, comm);
        break;
      case NBC_ARED_RING:
        res = allred_sched_ring(rank, p, count, datatype, sendbuf, recvbuf, op,
                                size, ext, schedule, handle, comm);
        break;
    }
    if (NBC_OK != res) {
    ...
    res = hb_tree_insert((hb_tree *)handle->comminfo->NBC_Dict[NBC_ALLREDUCE],
                         args, args, 0);
    if (res != 0)
      printf("error in dict_insert() (%i)\n", res);
    /* increase number of elements for A2A */
    if (++handle->comminfo->NBC_Dict_size[NBC_ALLREDUCE] >
        NBC_SCHED_DICT_UPPER) {
      NBC_SchedCache_dictwipe(
          (hb_tree *)handle->comminfo->NBC_Dict[NBC_ALLREDUCE],
          &handle->comminfo->NBC_Dict_size[NBC_ALLREDUCE]);
    }
  } else {
    /* found schedule */
...
static __inline__ int allred_sched_diss(int rank, int p, int count,
  ...
  root =
      0; /* this makes the code for ireduce and iallreduce nearly identical -
    ...
    if ((vrank % (1 << r)) == 0) {
      ...
      if (peer < p) {
        ...
        if (firstred) {
          ...
          res = NBC_Sched_op(recvbuf, false, sendbuf, false, 0, true, count,
                             datatype, op, schedule);
          firstred = 0;
        } else {
          /* perform the reduce in my local buffer */
          res = NBC_Sched_op(recvbuf, false, recvbuf, false, 0, true, count,
                             datatype, op, schedule);
        }
        if (res != NBC_OK) {
          free(handle->tmpbuf);
  ...
  for (r = 0; r < maxr; r++) {
    if (((vrank + (1 << r) < p) && (vrank < (1 << r))) || (vrank == 0)) {
      VRANK2RANK(peer, vrank + (1 << r), root);
      res =
          NBC_Sched_send(recvbuf, false, count, datatype, peer, schedule, comm);
      if (res != NBC_OK) {
        free(handle->tmpbuf);
        printf("Error in NBC_Sched_send() (%i)\n", res);
        return res;
...
static __inline__ int allred_sched_chain(int rank, int p, int count,
                                         ...
                                         NBC_Handle *handle, int fragsize,
                                         NBC_Comminfo *comm) {
  int res, rrpeer, rbpeer, srpeer, sbpeer, numfrag, fragnum, fragcount,
      thiscount, bstart, bend;
  ...
  bend = bstart + numfrag;
  /*printf("[%i] numfrag: %i, count: %i, size: %i, fragcount: %i, bstart: %i,
  ...
  for (fragnum = 0; fragnum < bend; fragnum++) {
    ...
    if (fragnum < numfrag) {
      ...
      if (rank != p - 1) {
        res = NBC_Sched_recv((char *)roffset, true, thiscount, datatype, rrpeer,
                             schedule, comm);
        if (NBC_OK != res) {
          printf("Error in NBC_Sched_recv() (%i)\n", res);
          return res;
        ...
        if (rank == 0) {
          res = NBC_Sched_op((char *)recvbuf + roffset, false,
                             (char *)sendbuf + roffset, false, (char *)roffset,
                             true, thiscount, datatype, op, schedule);
        } else {
          res = NBC_Sched_op((char *)roffset, true, (char *)sendbuf + roffset,
                             false, (char *)roffset, true, thiscount, datatype,
                             op, schedule);
        }
        res = NBC_Sched_barrier(schedule);
      }
      ...
      if (rank != 0) {
        ...
        if (rank == p - 1) {
          res = NBC_Sched_send((char *)sendbuf + roffset, false, thiscount,
                               datatype, srpeer, schedule, comm);
        } else {
          res = NBC_Sched_send((char *)roffset, true, thiscount, datatype,
                               srpeer, schedule, comm);
        }
        if (NBC_OK != res) {
          printf("Error in NBC_Sched_send() (%i)\n", res);
          return res;
    ...
    if (fragnum >= bstart) {
      ...
      if (rank != 0) {
        res = NBC_Sched_recv((char *)recvbuf + boffset, false, thiscount,
                             datatype, rbpeer, schedule, comm);
        if (NBC_OK != res) {
          printf("Error in NBC_Sched_recv() (%i)\n", res);
          return res;
        }
      ...
      if (rank != p - 1) {
        res = NBC_Sched_send((char *)recvbuf + boffset, false, thiscount,
                             datatype, sbpeer, schedule, comm);
        if (NBC_OK != res) {
          printf("Error in NBC_Sched_send() (%i)\n", res);
          return res;
        }
...
static __inline__ int allred_sched_ring(int r, int p, int count,
                                        MPI_Datatype datatype, void *sendbuf,
                                        void *recvbuf, MPI_Op op, int size,
                                        int ext, NBC_Schedule *schedule,
                                        NBC_Handle *handle,
  ...
  int i; /* runner */
  int segsize,
      *segsizes, *segoffsets; /* segment sizes and offsets per segment (number
                                 of segments == number of nodes */
   ...
   *   00       10       20      0: [1] -> 1
   *   01       11       21      1: [2] -> 2
   *   02       12       22      2: [0] -> 0  --> send element (r+1)%p to node
   * (r+1)%p
   *
   *      *** round 1 ***
   *    0        1        2
   ...
   *   02       12       22+12  2: red(2), [2] -> 0 --> reduce and send element
   * (r+0)%p to node (r+1)%p
   *
   *      *** round 2 ***
   *    0        1        2
   ...
   *   all      12       22/12  2: red(1), [1] -> 0 --> reduce and send (r-1)%p
   * to node (r+1)%p
   *
   *      *** round 3 ***
   *    0        1        2
   ...
   *   00/20    all      all    0: [1] -> 1
   *   all      11/01    all    1: [2] -> 2
   *   all      all      22/12  2: [0] -> 0 --> send element (r-2)%p to node
   * (r+1)%p
   *
   *      *** round 4 ***
   *    0        1        2
   ...
   *   01       11       21       31       1: [2] -> 2
   *   02       12       22       32       2: [3] -> 3
   *   03       13       23       33       3: [0] -> 0 --> send element (r+1)%p
   * to node (r+1)%p
   *
   *      *** round 1 ***
   *    0        1        2        3
   ...
   *   03       13       23       33+23    3: red(3), [3] -> 0 --> reduce and
   * send element (r+0)%p to node (r+1)%p
   *
   *      *** round 2 ***
   *    0        1        2        3
   *
   ...
   *   03+33/23 13       23       33/23    3: red(2), [2] -> 0 --> reduce and
   * send (r-1)%p to node (r+1)%p
   *
   *      *** round 3 ***
   *    0        1        2        3
   *
   ...
   *   03/33/23 all      23       33/23    3: red(1), [1] -> 0 --> reduce and
   * send (r-2)%p to node (r+1)%p
   *
   *      *** round 4 ***
   *    0        1        2        3
   *
   ...
   *   03/33/23 all      all      33/23    3: [0] -> 0 --> receive and send
   * element (r+1)%p to node (r+1)%p
   *
   *      *** round 5 ***
   *    0        1        2        3
   *
   ...
   *   03/33/23 all      all      all      3: [4] -> 4 --> receive and send
   * element (r-0)%p to node (r+1)%p
   *
   *      *** round 6 ***
   *    0        1        2        3
   *
   ...
   */
  {
    ...
    do {
      ...
      if (round == 0) {
        NBC_Sched_send((char *)sendbuf + soffset, false, segsizes[selement],
                       datatype, speer, schedule, comm);
        // printf("[%i] round %i - sending %i\n", r, round, selement);
      } else {
        NBC_Sched_send((char *)recvbuf + soffset, false, segsizes[selement],
                       datatype, speer, schedule, comm);
        // printf("[%i] round %i - sending %i\n", r, round, selement);
      }
      NBC_Sched_recv((char *)recvbuf + roffset, false, segsizes[relement],
                     datatype, rpeer, schedule, comm);
      // printf("[%i] round %i - receiving %i\n", r, round, relement);

      NBC_Sched_barrier(schedule);
      // printf("[%i] round %i - reducing %i\n", r, round, relement);
      NBC_Sched_op((char *)recvbuf + roffset, false, (char *)sendbuf + roffset,
                   false, (char *)recvbuf + roffset, false, segsizes[relement],
                   datatype, op, schedule);
      NBC_Sched_barrier(schedule);
