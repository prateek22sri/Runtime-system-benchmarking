...
void CHECK_AFFINITY(Domain *domain) {
  if (HPX_THREAD_ID != domain->lastCore) {
    // printf("Domain %d changed from worker %d to worker %d (%llu changes)\n",
    //        domain->rank, domain->last_worker, curr_worker,
    //        domain->worker_changes);
    domain->lastCore = HPX_THREAD_ID;
    domain->coreSwitches++;
  }
}

void CHECK_AFFINITY_SYNC(Domain *domain) {
  int prev = __atomic_exchange_n(&domain->lastCore, HPX_THREAD_ID, __ATOMIC_ACQ_REL);
  if (HPX_THREAD_ID != prev) {
    atomic_fetch_add_explicit(&domain->coreSwitches, 1, memory_order_acq_rel);
  }
}
           ...
           int nDomains, int maxcycles,Domain *domain)
{
  ...
  // initialize arrays
  for (i = 0; i < 26; ++i) {
    domain->dataRecvTT[i] = NULL;
    domain->dataRecvTF[i] = NULL;
    domain->dataRecvFF[i] = NULL;
  }
...
void DestroyDomain(Domain *domain)
{
  ...
  int i;
  for (i = 0; i < 26; i++) {
    if (domain->dataRecvTF[i])
      free(domain->dataRecvTF[i]);
    if (domain->dataRecvTT[i])
      free(domain->dataRecvTT[i]);
    if (domain->dataRecvFF[i])
