...
static __inline__ int bcast_sched_binomial(int rank, int p, int root,
                                           NBC_Schedule *schedule, void *buffer,
                                           int count, MPI_Datatype datatype,
                                           NBC_Comminfo *comm);
static __inline__ int bcast_sched_linear(int rank, int p, int root,
                                         NBC_Schedule *schedule, void *buffer,
                                         int count, MPI_Datatype datatype,
                                         NBC_Comminfo *comm);
static __inline__ int bcast_sched_chain(int rank, int p, int root,
                                        NBC_Schedule *schedule, void *buffer,
                                        int count, MPI_Datatype datatype,
                                        int fragsize, int size,
                                        NBC_Comminfo *comm);
...
int NBC_Bcast_args_compare(NBC_Bcast_args *a, NBC_Bcast_args *b, void *param) {
  ...
  if ((a->buffer == b->buffer) && (a->count == b->count) &&
      (a->datatype == b->datatype) && (a->root == b->root)) {
    return 0;
  }
...
int NBC_Ibcast(void *buffer, int count, MPI_Datatype datatype, int root,
               NBC_Comminfo *comminfo, NBC_Handle *handle) {
  ...
  search.root = root;
  found = (NBC_Bcast_args *)hb_tree_search(
      (hb_tree *)handle->comminfo->NBC_Dict[NBC_BCAST], &search);
  if (found == NULL) {
#endif
    ...
    if (res != NBC_OK) {
      printf("Error in NBC_Sched_create, res = %i\n", res);
      return res;
    }

    switch (alg) {
      case NBC_BCAST_LINEAR:
        res = bcast_sched_linear(rank, p, root, schedule, buffer, count,
                                 datatype, comminfo);
        break;
      case NBC_BCAST_BINOMIAL:
        res = bcast_sched_binomial(rank, p, root, schedule, buffer, count,
                                   datatype, comminfo);
        break;
      case NBC_BCAST_CHAIN:
        res = bcast_sched_chain(rank, p, root, schedule, buffer, count,
                                datatype, segsize, size, comminfo);
        break;
    }
    if (NBC_OK != res) {
    ...
    res = hb_tree_insert((hb_tree *)handle->comminfo->NBC_Dict[NBC_BCAST], args,
                         args, 0);
    if (res != 0)
      printf("error in dict_insert() (%i)\n", res);
    /* increase number of elements for A2A */
    if (++handle->comminfo->NBC_Dict_size[NBC_BCAST] > NBC_SCHED_DICT_UPPER) {
      NBC_SchedCache_dictwipe((hb_tree *)handle->comminfo->NBC_Dict[NBC_BCAST],
                              &handle->comminfo->NBC_Dict_size[NBC_BCAST]);
    }
  } else {
    /* found schedule */
...
static __inline__ int bcast_sched_binomial(int rank, int p, int root,
  ...
  for (r = 0; r < maxr; r++) {
    if (((vrank + (1 << r) < p) && (vrank < (1 << r))) || (vrank == 0)) {
      VRANK2RANK(peer, vrank + (1 << r), root);
      res =
          NBC_Sched_send(buffer, false, count, datatype, peer, schedule, comm);
      if (NBC_OK != res) {
