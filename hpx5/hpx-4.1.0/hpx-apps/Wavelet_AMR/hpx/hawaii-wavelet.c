...
void ext_func(const index_t *index, const int mask[n_variab + n_aux],
  ...
  for (int loop = 0; loop < 4; ++loop) {
    ...
    if (check_index(&stcl)) {
#ifdef HPX
      hpx_addr_t tmp_addr = hpx_get_coll_point(&stcl);
      hpx_addr_t u = hpx_addr_add(tmp_addr,gen*sizeof(vals[0]),sizeof(coll_point_t));
      hpx_gas_memget_sync(&vals[loop],u,sizeof(vals[0]));
#else
...
void wavelet_trans(coll_point_t *point, const wavelet_trans_t type, 
  ...
  if (interp_count == 1) {
    for (int loopa = 0; loopa < 4; ++loopa) {
      ...
      if (check_index(&stcl)) {
#ifdef HPX
        hpx_addr_t temp = hpx_get_coll_point(&stcl);
        hpx_addr_t u = hpx_addr_add(temp,gen*sizeof(buffer),sizeof(coll_point_t));
        hpx_gas_memget_sync(&buffer,u,sizeof(buffer));
        contrib = buffer;
#else
        coll_point_t *stcl_point = get_coll_point(&stcl);
