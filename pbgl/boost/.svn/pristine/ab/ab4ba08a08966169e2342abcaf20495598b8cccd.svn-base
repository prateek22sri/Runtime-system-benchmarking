...
double run_sssp(amplusplus::transport &trans, amplusplus::transport &barrier_trans, 
		Graph& g,	typename graph_traits<Graph>::vertex_descriptor current_source,
		//typename property_map<Graph, int VertexProperties::*>::type & distance,
		...
		typename graph_traits<Graph>::vertices_size_type n, bool verify)
{
  ...
  BGL_FORALL_VERTICES_T(v, g, Graph) {
    ...
    //put(distance, v, (std::numeric_limits<double>::max)());
...
/*  boost::graph::distributed::delta_stepping_shortest_paths<
		Graph, sssp_visitor<
			distance_recorder<DistanceMap, on_tree_edge> 
		> > sssp(g, make_sssp_visitor(record_distances(distance, on_tree_edge())), color); */

  boost::graph::distributed::dijkstra_shortest_paths<Graph,
			...
			typename boost::property_map<Graph, weight_type WeightedEdge::*>::type>
				sssp(g, start, distance /*get(&VertexProperties::distance, g)*/, get(&WeightedEdge::weight, g));
  ...
  BGL_FORALL_VERTICES_T(v, g, Graph) {
	...
	//if (get(distance, v) < (std::numeric_limits<double>::max)())
			++visited;
  }
	  ...
	  if (trans.rank() == 0) {
		...
		{ amplusplus::scoped_epoch epoch(trans); 
			...
			for (uint64_t i = 0; i < e_count; i++) {
				...
				//if (trans.rank() == 0)
				//property<edge_weight_t, double> edge_prop;
				//WeightedEdge edge_prop;
  ...
  DistanceMap distance = get(&VertexProperties::distance, g);
	...
	for (unsigned int source_i = 0; source_i < num_sources; ++source_i) {
		...
		//{ amplusplus::scoped_epoch epoch(trans); 
			...
			double sssp_time = 
				run_sssp<Graph>(trans, barrier_trans, g, current_source, distance, n, verify);
