...
public:
  ...
  //int *destinationPes;
  //dtype *dataItems;
	std::vector<int> destinationPes;
	std::vector<dtype> dataItems;

public:
  MeshStreamerMessage(): numDataItems(0), msgType(0) {
    finalMsgCount = -1;
  }

  MeshStreamerMessage(/*int maxDataItems,*/ int t): numDataItems(0), msgType(t) {
    finalMsgCount = -1;
		destinationPes.resize(CkpvAccess(_offsets)[1]); //FIXME!
		dataItems.resize(CkpvAccess(_offsets)[2]);   //FIXME!
  }

	void* operator new(size_t, void*p) { return p; }
	void * operator new(size_t s, int sz0, int sz1, const int p) {
		// use CkpvAccess(_offsets)[] to pass sizes of vectors to constructor
		CkpvAccess(_offsets)[0] = ALIGN_DEFAULT(s);
		CkpvAccess(_offsets)[1] = sz0;//not used
		CkpvAccess(_offsets)[2] = sz1;//not used
		return CkAllocMsg(CMessage_MeshStreamerMessage<dtype>::__idx, CkpvAccess(_offsets)[0], p);
		//return NULL;
		//return MeshStreamerMessage < dtype > ::alloc(__idx, s, 0, p);
	}

	// FIXME:!!
	/*void operator delete(void *p, int sz0, int sz1, const int p) {
		dealloc(p);
	}*/
  ...
  inline const dtype& getDataItem(const int index) {
	...
	static void *pack(MeshStreamerMessage *m) {
		//CkAbort("MeshStreamerMeshStreamerMessage::pack called");
		// count total size of the message
		int totalsize = 3 * sizeof(int) + (m->destinationPes.size() + 1) * sizeof(int);
		for (int i = 0; i < m->numDataItems; i++)
			totalsize += m->dataItems[i].size();
		totalsize += sizeof(int); // tail (for debug only)
		...
		char *p = buf;
		*(int *)p = m->finalMsgCount;
		p += sizeof(int);
		*(int *)p = m->msgType;
		p += sizeof(int);
		*(int *)p = m->numDataItems;
		p += sizeof(int);

		*(int *)p = m->destinationPes.size();
		p += sizeof(int);
		memcpy(p, &(m->destinationPes[0]), m->destinationPes.size() * sizeof(int));
		p += m->destinationPes.size() * sizeof(int);

		for (int i = 0; i < m->numDataItems; i++) {
			m->dataItems[i].pack((void *)p);
			p += m->dataItems[i].size();
		}
	...
	static MeshStreamerMessage *unpack(void *buf) {
		// ?

		//MeshStreamerMessage *m = new (1,0,0) MeshStreamerMessage();
		MeshStreamerMessage *m = 
			(MeshStreamerMessage *)CkAllocBuffer(buf, sizeof(MeshStreamerMessage));
		m = new ((void *)m) MeshStreamerMessage<dtype>();

		char *p = (char *)buf;
		m->finalMsgCount = *(int *)p;
		p += sizeof(int);
		m->msgType = *(int *)p;
		p += sizeof(int);
		m->numDataItems = *(int *)p;
		p += sizeof(int);

		int size = *(int *)p;
		p += sizeof(int);
 		m->destinationPes.resize(size);
		memcpy(&(m->destinationPes[0]), p, size * sizeof(int));

		m->dataItems.resize(m->numDataItems);
		for (int i = 0; i < m->numDataItems; i++) {
			m->dataItems[i].unpack((void *)p);
			p += m->dataItems[i].size();
		}
...
flushDimension(int dimension, bool sendMsgCounts) {
  ...
  for (int j = 0; j < messageBuffers.size(); j++) {
    ...
    if(messageBuffers[j] == NULL && sendMsgCounts) {
			//CkAbort("MeshStreamerMessage::flushDimension check");
...
public:
  ...
  inline void insertData(const dtype& dataItem, itype arrayIndex) {
		...
		if (arrayIndex < 0)
    CkAssert(arrayIndex >= 0);

    // no data items should be submitted after all local contributors call done
    // and staged completion has begun
		//if (this->stagedCompletionStarted_ == true)
		//	while(1) {}
