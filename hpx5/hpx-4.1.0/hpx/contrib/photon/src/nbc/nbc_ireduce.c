...
red_sched_binomial(int rank, int p, int root, void *sendbuf, void *recvbuf,
                   int count, MPI_Datatype datatype, MPI_Op op, void *redbuf,
                   NBC_Schedule *schedule, NBC_Handle *handle,
                   NBC_Comminfo *comm);
static __inline__ int red_sched_chain(int rank, int p, int root, void *sendbuf,
                                      void *recvbuf, int count,
                                      MPI_Datatype datatype, MPI_Op op, int ext,
                                      int size, NBC_Schedule *schedule,
                                      NBC_Handle *handle, int fragsize,
                                      NBC_Comminfo *comm);

#ifdef NBC_CACHE_SCHEDULE
/* tree comparison function for schedule cache */
int NBC_Reduce_args_compare(NBC_Reduce_args *a, NBC_Reduce_args *b,
                            void *param) {

  if ((a->sendbuf == b->sendbuf) && (a->recvbuf == b->recvbuf) &&
      (a->count == b->count) && (a->datatype == b->datatype) &&
      (a->op == b->op) && (a->root == b->root)) {
    return 0;
  }
...
int NBC_Ireduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
  ...
  search.op = op;
  search.root = root;
  found = (NBC_Reduce_args *)hb_tree_search(
      (hb_tree *)handle->comminfo->NBC_Dict[NBC_REDUCE], &search);
  if (found == NULL) {
#endif
    ...
    switch (alg) {
      case NBC_RED_BINOMIAL:
        res = red_sched_binomial(rank, p, root, sendbuf, recvbuf, count,
                                 datatype, op, redbuf, schedule, handle, comm);
        break;
      case NBC_RED_CHAIN:
        res = red_sched_chain(rank, p, root, sendbuf, recvbuf, count, datatype,
                              op, ext, size, schedule, handle, segsize, comm);
        break;
    }
    if (NBC_OK != res) {
    ...
    res = hb_tree_insert((hb_tree *)handle->comminfo->NBC_Dict[NBC_REDUCE],
                         args, args, 0);
    if (res != 0)
    ...
    if (++handle->comminfo->NBC_Dict_size[NBC_REDUCE] > NBC_SCHED_DICT_UPPER) {
      NBC_SchedCache_dictwipe((hb_tree *)handle->comminfo->NBC_Dict[NBC_REDUCE],
                              &handle->comminfo->NBC_Dict_size[NBC_REDUCE]);
    }
  } else {
    /* found schedule */
  ...
  for (r = 1; r <= maxr; r++) {
    if ((vrank % (1 << r)) == 0) {
      ...
      if (peer < p) {
        ...
        if (firstred) {
          if (rank == root) {
             ...
             * take data from sendbuf in first round - save copy */
            res = NBC_Sched_op((char *)redbuf - (unsigned long)handle->tmpbuf,
                               true, sendbuf, false, 0, true, count, datatype,
                               op, schedule);
          }
          firstred = 0;
        } else {
          if (rank == root) {
            ...
            res = NBC_Sched_op((char *)redbuf - (unsigned long)handle->tmpbuf,
                               ...
                               true, 0, true, count, datatype, op, schedule);
          }
        }
        if (NBC_OK != res) {
          free(handle->tmpbuf);
      ...
      if (firstred) {
        ...
        /* we have to use the redbuf the root (which works in receivebuf) is
         * never sending .. */
        res = NBC_Sched_send((char *)redbuf - (unsigned long)handle->tmpbuf,
                             true, count, datatype, peer, schedule, comm);
      }
      if (NBC_OK != res) {
        free(handle->tmpbuf);
...
static __inline__ int red_sched_chain(int rank, int p, int root, void *sendbuf,
  ...
  for (fragnum = 0; fragnum < numfrag; fragnum++) {
    ...
    if (vrank != p - 1) {
      res = NBC_Sched_recv((char *)offset, true, thiscount, datatype, rpeer,
                           schedule, comm);
      if (NBC_OK != res) {
        printf("Error in NBC_Sched_recv() (%i)\n", res);
        return res;
      ...
      if (vrank == 0) {
        res = NBC_Sched_op((char *)recvbuf + offset, false,
                           (char *)sendbuf + offset, false, (char *)offset,
                           true, thiscount, datatype, op, schedule);
      } else {
        res = NBC_Sched_op((char *)offset, true, (char *)sendbuf + offset,
                           false, (char *)offset, true, thiscount, datatype, op,
                           schedule);
      }
      res = NBC_Sched_barrier(schedule);
    }
    ...
    if (vrank != 0) {
      ...
      if (vrank == p - 1) {
        res = NBC_Sched_send((char *)sendbuf + offset, false, thiscount,
                             datatype, speer, schedule, comm);
      } else {
        res = NBC_Sched_send((char *)offset, true, thiscount, datatype, speer,
                             schedule, comm);
      }
      if (NBC_OK != res) {
        printf("Error in NBC_Sched_send() (%i)\n", res);
