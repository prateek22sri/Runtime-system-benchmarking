...
void ReportResults(const SparseMatrix & A, int numberOfMgLevels, int numberOfCgSets, int refMaxIters,int optMaxIters, double times[],
		const TestCGData & testcg_data, const TestSymmetryData & testsymmetry_data, const TestNormsData & testnorms_data, int global_failure,Domain *domain) {
  ...
  dmin_reduction(&t4min,t4,domain->reduce.dreduction_min,domain->reduce.dallreduce_min,domain->params.comm_rank);
  computeresidual_reduction(&t4max,t4,domain->reduce.dreduction_max,
                            domain->reduce.dallreduce_max,domain->params.comm_rank);
  dotproduct_reduction(&t4avg,t4,
         domain->reduce.dreduction_sum,domain->reduce.dallreduce,domain->params.comm_rank);
