...
typedef enum { SEND, RECV, OP, COPY, UNPACK } NBC_Fn_type;
...
int NBC_Allgather_args_compare(NBC_Allgather_args *a, NBC_Allgather_args *b,
                               void *param);
...
int NBC_Gather_args_compare(NBC_Gather_args *a, NBC_Gather_args *b,
                            void *param);
...
int NBC_Scatter_args_compare(NBC_Scatter_args *a, NBC_Scatter_args *b,
                             void *param);
...
static __inline__ int NBC_Copy(void *src, int srccount, MPI_Datatype srctype,
 ...
 * [schedule] ::=
 * [size][round-schedule][delimiter][round-schedule][delimiter]...[end]
 ...
 * [type-args] ::= type specific arguments (NBC_Args_send, NBC_Args_recv or,
 * NBC_Args_op)
 ...
 * [num]{[type][type-args]}
 * e.g.
 * [(int)2][(NBC_Fn_type)SEND][(NBC_Args_send)SEND-ARGS][(NBC_Fn_type)RECV][(NBC_Args_recv)RECV-ARGS]
 */
...
#define NBC_PRINT_ROUND(schedule)                                              \
  {                                                                            \
    ...
    for (i = 0; i < *numptr; i++) {                                            \
      ...
      switch (*typeptr) {                                                      \
        case SEND:                                                             \
          printf("[%i]  SEND (offset %li) ", myrank,                           \
                 (long)typeptr - (long)schedule);                              \
          sendargs = (NBC_Args_send *)(typeptr + 1);                           \
          printf("*buf: %lu, count: %i, type: %lu, dest: %i)\n",               \
                 (unsigned long)sendargs->buf, sendargs->count,                \
                 (unsigned long)sendargs->datatype, sendargs->dest);           \
          typeptr = (NBC_Fn_type *)((NBC_Args_send *)typeptr + 1);             \
          break;                                                               \
        case RECV:                                                             \
          printf("[%i]  RECV (offset %li) ", myrank,                           \
                 (long)typeptr - (long)schedule);                              \
          recvargs = (NBC_Args_recv *)(typeptr + 1);                           \
          printf("*buf: %lu, count: %i, type: %lu, source: %i)\n",             \
                 (unsigned long)recvargs->buf, recvargs->count,                \
                 (unsigned long)recvargs->datatype, recvargs->source);         \
          typeptr = (NBC_Fn_type *)((NBC_Args_recv *)typeptr + 1);             \
          break;                                                               \
        case OP:                                                               \
          printf("[%i]  OP   (offset %li) ", myrank,                           \
                 (long)typeptr - (long)schedule);                              \
          opargs = (NBC_Args_op *)(typeptr + 1);                               \
          printf("*buf1: %lu, buf2: %lu, count: %i, type: %lu)\n",             \
                 (unsigned long)opargs->buf1, (unsigned long)opargs->buf2,     \
                 opargs->count, (unsigned long)opargs->datatype);              \
          typeptr = (NBC_Fn_type *)((NBC_Args_op *)typeptr + 1);               \
          break;                                                               \
        case COPY:                                                             \
          printf("[%i]  COPY   (offset %li) ", myrank,                         \
                 (long)typeptr - (long)schedule);                              \
          ...
          break;                                                               \
        case UNPACK:                                                           \
          printf("[%i]  UNPACK   (offset %li) ", myrank,                       \
                 (long)typeptr - (long)schedule);                              \
...
#define NBC_PRINT_SCHED(schedule)                                              \
  {                                                                            \
    ...
    while ((long)ptr - (long)schedule < size) {                                \
      NBC_GET_ROUND_SIZE(ptr, round_size);                                     \
      printf("[%i] Round at byte %li (size %li) ", myrank,                     \
             (long)ptr - (long)schedule, round_size);                          \
      NBC_PRINT_ROUND(ptr);                                                    \
      /* add round size */                                                     \
      ptr = ptr + round_size;                                                  \
      /* add sizeof(char) as barrier delimiter */                              \
...
static __inline__ int NBC_Type_intrinsic(MPI_Datatype type) {
  ...
  if ((type == MPI_INT) || (type == MPI_LONG) || (type == MPI_SHORT) ||
      (type == MPI_UNSIGNED) || (type == MPI_UNSIGNED_SHORT) ||
      (type == MPI_UNSIGNED_LONG) || (type == MPI_FLOAT) ||
      (type == MPI_DOUBLE) || (type == MPI_LONG_DOUBLE) || (type == MPI_BYTE) ||
      (type == MPI_FLOAT_INT) || (type == MPI_DOUBLE_INT) ||
      (type == MPI_LONG_INT) || (type == MPI_2INT) || (type == MPI_SHORT_INT) ||
...
static __inline__ int NBC_Unpack(void *src, int srccount, MPI_Datatype srctype,
                                 void *tgt) {
  int res;
  MPI_Aint ext;
...
static __inline__ void NBC_SchedCache_dictwipe(hb_tree *dict, int *size) {
  ...
  for (; hb_itor_valid(itor) && (*size > NBC_SCHED_DICT_LOWER);
       hb_itor_next(itor)) {
