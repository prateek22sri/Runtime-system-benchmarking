...
#ifdef CILK
  int skipit = sync_fadd(&point->stage[RHS_HLLE], 1, SYNC_ACQ_REL);
#else
...
#ifdef CILK
        int skipp2c = sync_fadd(&stcl_point->stage[RHS_HLLE], 1, SYNC_ACQ_REL);
#else
...
#ifdef CILK
  int skipit = sync_fadd(&point->stage[RHS_FLUX], 1, SYNC_ACQ_REL);
#else
