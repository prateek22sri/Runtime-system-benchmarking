...
struct SparseMatrix_STRUCT {
  ...
  int * grecv_length; //!< receive length
  int * rn_order;
};
...
/*!
  ...
  @param[in] A the known system matrix
 */
inline void DeleteMatrix(SparseMatrix & A,int rank) {
  ...
  if (A.Ac!=0) { DeleteMatrix(*A.Ac,rank); delete A.Ac; A.Ac = 0;} // Delete coarse matrix
  if (A.mgData!=0) { DeleteMGData(*A.mgData,rank); delete A.mgData; A.mgData = 0;} // Delete MG data
  return;
}
