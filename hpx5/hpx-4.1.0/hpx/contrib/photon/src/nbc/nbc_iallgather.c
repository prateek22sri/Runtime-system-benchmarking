 ...
 * each node sends the packet it received last round (or has in round 0) to it's
 * right neighbor (modulo p)
...
int NBC_Iallgather(void *sendbuf, int sendcount, MPI_Datatype sendtype,
  ...
  found = (NBC_Allgather_args *)hb_tree_search(
      (hb_tree *)handle->comminfo->NBC_Dict[NBC_ALLGATHER], &search);
  if (found == NULL) {
#endif
    ...
    for (r = 0; r < p; r++) {
      if (r != rank) {
        ...
        res =
            NBC_Sched_recv(rbuf, false, recvcount, recvtype, r, schedule, comm);
        if (NBC_OK != res) {
        ...
        res =
            NBC_Sched_send(sbuf, false, recvcount, recvtype, r, schedule, comm);
        if (NBC_OK != res) {
    ...
    res = hb_tree_insert((hb_tree *)handle->comminfo->NBC_Dict[NBC_ALLGATHER],
                         args, args, 0);
    if (res != 0)
      printf("error in dict_insert() (%i)\n", res);
    /* increase number of elements for A2A */
    if (++handle->comminfo->NBC_Dict_size[NBC_ALLGATHER] >
        NBC_SCHED_DICT_UPPER) {
      NBC_SchedCache_dictwipe(
          (hb_tree *)handle->comminfo->NBC_Dict[NBC_ALLGATHER],
          &handle->comminfo->NBC_Dict_size[NBC_ALLGATHER]);
    }
  } else {
    /* found schedule */
